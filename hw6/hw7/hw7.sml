...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
	  ...
	  |Shift(r1,r2,ex) => case eval_prog(ex,env) of  
	                       NoPoints => NoPoints
						   |Point(x,y) => Point(r1 + x, r2 + y)
						   |Line(m,b) => Line(m, (b + r2) - (m * r1))
						   |VerticalLine(x) => VerticalLine(r1 + x) 
						   |LineSegment(x1,y1,x2,y2) => LineSegment(x1 + r1, y1 + r2, x2 + r1, y2 + r2)
						   |Shift(r1,r2,ex) => eval_prog (Shift(r1,r2,eval_prog(ex,env)), env)
						   |Var(s) => eval_prog(Shift(r1,r2,eval_prog(Var(s),env)),env)
						   |Let(s,e1,e2) => eval_prog(Shift (r1,r2,eval_prog(Let(s,e1,e2),env)),env)
                           |Intersect(e1,e2) => eval_prog(Shift (r1,r2,eval_prog(Intersect(e1,e2),env)),env)
(* CHANGE: Add a case for Shift expressions *)

(* CHANGE: Add function preprocess_prog of type geom_exp -> geom_exp *)
fun preprocess_prog gexp = 
 case gexp of 
 LineSegment (x1,y1,x2,y2) => if real_close_point (x1,y1) (x2,y2) 
                              then Point(x1,y1)
							       ...
							       then LineSegment(x1,y1,x2,y2)
								        ...
								        then  LineSegment(x1,y1,x2,y2)
										else preprocess_prog(LineSegment(x2,y2,x1,y1))
    
	 | Intersect(ex1,ex2) =>  Intersect(preprocess_prog(ex1),preprocess_prog(ex2))
	 | Let(s,ex1,ex2) => Let(s, preprocess_prog(ex1),preprocess_prog(ex2)) 
	 | Shift  (r1, r2, ex) => Shift(r1,r2, preprocess_prog(ex))
	 | _ => gexp
